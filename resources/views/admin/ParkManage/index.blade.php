            ...
            <div class="animate__animated p-6" :class="[$store.app.animation]">
                ...
                <div x-data="contacts">
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        <h2 class="text-xl">Parking Management</h2>
                        <div class="flex w-full flex-col sm:w-auto sm:flex-row sm:items-center sm:gap-3">
                            <div class="flex gap-3 items-center">
                                <!-- Add Parking Button -->
                                <form action="{{ route('parkings.create') }}" method="GET">
                                ...
                                <!-- Search Bar -->
                                <div class="">
                                    <input type="text" id="searchBar" placeholder="Search Parking Records..."
                                        class="form-control rounded-lg border border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 px-4 py-2 w-80 text-gray-700 bg-black text-white"
                                        oninput="filterTable()">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="panel mt-5 overflow-hidden border-0 p-0">
                        <template x-if="displayType === 'list'">
                            <div class="table-responsive" style="max-height: 400px; overflow-y-auto;">
                                ...
                                <table class="table-striped table-hover min-w-full">
                                    <thead>
                                        <tr>
                                            <th>Parking Code</th>
                                            <th>Check In</th>
                                    ...
                                    <tbody id="parkingTableBody">
                                        @foreach ($parkings as $parking)
                                            <tr data-parking-id="{{ $parking->id }}">
                                                <td class="whitespace-nowrap">{{ $parking->parking_code }}</td>
                                                <td class="whitespace-nowrap">{{ $parking->check_in }}</td>
                                                <td class="whitespace-nowrap">{{ $parking->check_out ?? 'N/A' }}</td>
                                                <td class="whitespace-nowrap">{{ $parking->vehicle_type }}</td>
                                                <td class="whitespace-nowrap">{{ $parking->rate_name }}</td>
                                                <td class="whitespace-nowrap">{{ $parking->slot }}</td>
                                                <td class="whitespace-nowrap">{{ $parking->total_time }} hours</td>
                                                <td class="whitespace-nowrap">
                                                    ₱{{ number_format($parking->total_amount, 2) }}</td>
                                                <td class="whitespace-nowrap">
                                                    @if (strtolower($parking->paid_status) === 'paid')
                                                    ...
                                                    <div class="flex items-center justify-center gap-4">
                                                        @if (strtolower($parking->paid_status) === 'paid')
                                                            <button type="button"
                                                                ...
                                                                onclick="printParkingDetails({{ $parking->id }})">
                                                                Print
                                                            </button>
                                                        @endif
                                                        @if (strtolower($parking->paid_status) !== 'paid')
                                                        ...
                                                        @endif
                                                        <form action="{{ route('parkings.destroy', $parking->id) }}"
                                                            method="POST" style="display: inline;">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="btn btn-sm btn-outline-danger"
                                                                onclick="return confirm('Are you sure you want to delete this parking record?')">
                                                                Delete
                                                            </button>
                                                        </form>
                                                    </div>
                                ...
                                <script>
                                    ...
                                    function printParkingDetails(parkingId) {
                                        const row = document.querySelector(`tr[data-parking-id="${parkingId}"]`);

                                        if (!row) {
                                            alert("Could not find parking details to print.");
                                            return;
                                        }

                                        const cells = row.querySelectorAll("td");
                                        const details = {
                                            parkingCode: cells[0]?.textContent.trim(),
                                            checkIn: cells[1]?.textContent.trim(),
                                            checkOut: cells[2]?.textContent.trim() || "N/A",
                                            vehicleType: cells[3]?.textContent.trim(),
                                            rateName: cells[4]?.textContent.trim(),
                                            slot: cells[5]?.textContent.trim(),
                                            totalTime: cells[6]?.textContent.trim(),
                                            totalAmount: cells[7]?.textContent.trim(),
                                            paidStatus: cells[8]?.textContent.trim(),
                                        };
        ...
        <html>
            ...
            <body>
                <div class="receipt">
                    ...
                    <div class="receipt-item"><span class="label">Parking Code:</span> ${details.parkingCode}</div>
                    <div class="receipt-item"><span class="label">Check In:</span> ${details.checkIn}</div>
                    <div class="receipt-item"><span class="label">Check Out:</span> ${details.checkOut}</div>
                    <div class="receipt-item"><span class="label">Vehicle Type:</span> ${details.vehicleType}</div>
                    <div class="receipt-item"><span class="label">Rate Name:</span> ${details.rateName}</div>
                    <div class="receipt-item"><span class="label">Slot:</span> ${details.slot}</div>
                    <div class="receipt-item"><span class="label">Total Time:</span> ${details.totalTime}</div>
                    <div class="receipt-item"><span class="label">Total Amount:</span> ${details.totalAmount}</div>
                    <div class="receipt-item"><span class="label">Paid Status:</span> ${details.paidStatus}</div>
